...
if __name__ == "__main__":
    ...
    # Chemins pour les fichiers temporaires et finaux
    project_root = Path(__file__).parent.parent
    tmp_dir = project_root / "docs" / "models" / model_name / "tmp_save"
    final_dir = project_root / "docs" / "models" / model_name
    ...
    for i in (t:=trange(getenv("STEPS", 200))):
        ...
        if (i % 10 == 9) and (test_acc := get_test_acc().item()) > best_acc:
            best_since = 0
            best_acc = test_acc
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S%f')  # microsecondes pour nom unique
            state_dict = get_state_dict(model)
            new_file = tmp_dir / f"{model_name}_{timestamp}.safetensors"
            try:
        ...
        # Chargement du meilleur modèle si patience atteinte
    ...
    # Maintenant on peut changer le device et créer un nouveau modèle pour l'export
    Device.DEFAULT = "WEBGPU"
    model_webgpu = Model()
    load_state_dict(model_webgpu, state_dict)
    
    input = Tensor.randn(1, 1, 28, 28)
    prg, *_, state = export_model(model_webgpu, Device.DEFAULT.lower(), input, model_name=model_name)
    safe_save(state, final_dir / f"{model_name}.webgpu.safetensors")
    with open(final_dir / f"{model_name}.js", "w") as text_file:
        text_file.write(prg)